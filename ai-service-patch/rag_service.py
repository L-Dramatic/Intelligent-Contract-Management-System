...
class RAGService:
    ...
    def __init__(self, chroma_db_path: str = None, collection_name: str = "telecom_contracts"):
        ...
        # Embedding模型支持两种配置方式：
        # 1) EMBEDDING_MODEL_PATH：离线本地模型目录（推荐，服务器无外网时必须）
        # 2) EMBEDDING_MODEL_NAME：HuggingFace模型名（需要可访问huggingface）
        self.embedding_model_name = os.getenv(
            "EMBEDDING_MODEL_NAME",
            "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
        )
        self.embedding_model_path = os.getenv("EMBEDDING_MODEL_PATH", "").strip() or None
        self.embedding_model_id = self.embedding_model_path or self.embedding_model_name
