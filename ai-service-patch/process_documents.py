...
EMBEDDING_MODEL_ID = EMBEDDING_MODEL_PATH or EMBEDDING_MODEL_NAME
CHUNK_SIZE = 500
CHUNK_OVERLAP = 50
...
def process_documents():
    ...
    print(f"[2] 加载Embedding模型: {EMBEDDING_MODEL_ID}")
    try:
        ...
        print("    解决方案：")
        print("    - 若服务器无外网：先下载模型并上传到服务器，然后设置环境变量 EMBEDDING_MODEL_PATH 指向模型目录")
        print("    - 或确保服务器可访问 HuggingFace，再使用默认模型名")
        sys.exit(1)
