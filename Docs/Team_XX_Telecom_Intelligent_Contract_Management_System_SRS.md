...
- 审批任务状态已更新
- 如果流程流转，下一审批人收到通知
- 如果内部流程结束，合同状态更新为"待对方签署"，系统自动向对方联系人发送签署链接/验证码。
...
**业务背景**：
本系统采用"组织树 + 权重"的审批逻辑。管理员需构建电信运营商内部的组织层级，并为关键职位配置审批额度，以便工作流引擎自动计算审批路径。

**基本流程**：
1.  管理员进入"组织架构管理"模块。
2.  构建层级：管理员创建根节点（如"省公司"），并逐级添加子部门/职位节点（如"市公司"->"网络部"->"网络建设经理"）。
3.  配置权重（核心）：
    * 管理员选中某个职位节点（如"市公司分管副总"）。
    * 设置审批权重 (ApprovalLimit)：例如输入 "500,000"。
    * *含义：该职位有权终审金额在50万元以下的合同。*
4.  调整结构：支持拖拽调整部门归属关系。
5.  系统自动更新树状路径编码（Path），确保审批流转的上下级关系准确。

**后置条件**：
* 内部组织树已构建，各节点审批权重已配置，可用于工作流引擎的路径计算。

---

#### 4.3.2 用例UC-10：内部员工职位绑定
...
**业务背景**：
电信公司员工数量众多（数千人），且存在一人多岗情况。系统需将已存在的员工账号（SSO/LDAP同步）准确绑定到组织树的具体职位节点上，以获得相应的审批权限。

**基本流程**：

**【阶段一：批量导入与邀请】**
1.  管理员下载"职位绑定模板"（Excel）。
2.  填写员工信息：姓名、工号、手机号、对应组织树节点ID。
3.  管理员上传文件，系统解析并校验工号有效性。
4.  系统生成职位激活码，并通过内部短信/邮件发送给员工。
    * *注意：此时员工账号与职位尚未正式绑定，状态为"待激活"等待员工确认。*

**【阶段二：员工激活身份】**

5.  员工登录系统（使用内部账号）。
6.  系统提示"您有新的职位待激活"，员工输入收到的6位激活码。
7.  系统验证通过，正式建立"用户-组织节点"的绑定关系。
8.  员工即刻获得该职位对应的审批权重和数据权限。

**关键规则**：
* BR-091 内部专用：此功能仅针对电信公司内部员工。外部人员（如基站业主、供应商）不作为系统用户管理，仅作为合同中的"联系人信息"存储，通过临时链接交互。
* BR-092 离职/调岗处理：员工调岗时，管理员仅需解绑旧节点、邀请绑定新节点，无需删除账号。

**后置条件**：
* 员工账号已关联到具体的组织树节点，具备了发起或审批合同的资格。

#### 4.3.3 用例UC-11：管理电信领域知识库
...
**核心需求**：
- FR-110：系统应提供用户名密码登录功能
- FR-111：系统应使用JWT实现无状态认证
- FR-112：系统应实现RBAC权限模型
- FR-113：系统应支持流程管理员角色
...
**组织树节点 (OrganizationTreeNode)**

| 字段 | 说明 |
|------|------|
| 节点ID | 主键 |
| 职位名称 | 如"总经理"、"网络部经理" |
| 父节点ID | 上级职位节点ID（构建树状结构） |
| 审批权重 (ApprovalLimit) | 该职位可终审的最大金额（单位：元） |
| 所属部门 | 部门标识 |
| 层级路径 | 用于快速检索的树路径字符串 |
